...
namespace Model
{
	...
	std::shared_ptr< AbstractSensor> AbstractAgent::getSensor(uint8_t index)
	{
		return sensors.at(index);
	}
			
	std::shared_ptr< AbstractActuator> AbstractAgent::getActuator(uint8_t index)
	{
		if(actuators.size() > 0)
		{
			return actuators.at(index);
		}
		return nullptr;
	}
	/**
