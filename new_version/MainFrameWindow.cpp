...
namespace Application
{
	...
	Panel* MainFrameWindow::initialiseButtonPanel()
	{
		...
		sizer->Add( makeButton( panel,
								"Start Particle robot",
								[this](CommandEvent &anEvent){this->OnStartRobotParticle(anEvent);}),
					GBPosition( 1, 0),
					GBSpan( 1, 1), EXPAND);
		sizer->Add( makeButton( panel,
								"Start Kalman robot",
								[this](CommandEvent &anEvent){this->OnStartRobotKalman(anEvent);}),
					GBPosition( 1, 1),
					GBSpan( 1, 1), EXPAND);
		sizer->Add( makeButton( panel,
								"Start Both Particle robot",
								[this](CommandEvent &anEvent){this->OnStartRobotBothParticle(anEvent);}),
					GBPosition( 2, 0),
					GBSpan( 1, 1), EXPAND);
		sizer->Add( makeButton( panel,
								"Start Both Kalman robot",
								[this](CommandEvent &anEvent){this->OnStartRobotBothKalman(anEvent);}),
					GBPosition( 2, 1),
					GBSpan( 1, 1), EXPAND);
	...
	void MainFrameWindow::OnStartRobot( CommandEvent& UNUSEDPARAM(anEvent))
	{
		...
		{
			robot->setFilter(Model::FILTERS::KALMAN);
			robot->startActing();
		}
	}
	void MainFrameWindow::OnStartRobotKalman( CommandEvent& UNUSEDPARAM(anEvent))
	{
		Logger::log( "Attempting to start Robot...");
		Model::RobotPtr robot = Model::RobotWorld::getRobotWorld().getRobot( "Robot");
		if (robot && !robot->isActing())
	...
	void MainFrameWindow::OnStartRobotParticle( CommandEvent& UNUSEDPARAM(anEvent))
	{
		Logger::log( "Attempting to start Robot...");
		Model::RobotPtr robot = Model::RobotWorld::getRobotWorld().getRobot( "Robot");
		if (robot && !robot->isActing())
		{
			robot->setFilter(Model::FILTERS::PARTICLE);
			robot->startActing();
		}
	}
	void MainFrameWindow::OnStartRobotBothKalman( CommandEvent& UNUSEDPARAM(anEvent))
	{
		Logger::log( "Attempting to start Robot...");
		Model::RobotPtr robot = Model::RobotWorld::getRobotWorld().getRobot( "Robot");
		if (robot && !robot->isActing())
		{
			robot->setFilter(Model::FILTERS::BOTHKALMAN);
			robot->startActing();
		}
	}
	void MainFrameWindow::OnStartRobotBothParticle( CommandEvent& UNUSEDPARAM(anEvent))
	{
		Logger::log( "Attempting to start Robot...");
		Model::RobotPtr robot = Model::RobotWorld::getRobotWorld().getRobot( "Robot");
		if (robot && !robot->isActing())
		{
			robot->setFilter(Model::FILTERS::BOTHPARTICLE);
			robot->startActing();
		}
	}
	/**
