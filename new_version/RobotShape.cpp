...
//		}
	...
	void RobotShape::draw( wxDC& dc)
	{
		...
		std::vector<Point> rpath = getRobot()->getRealPath();
		dc.SetPen(wxPen(WXSTRING("BLACK"),1, wxPENSTYLE_SOLID));
		for(uint64_t i = 1 ; i < rpath.size();++i)
		{
			dc.DrawLine(rpath.at(i-1).x,rpath.at(i-1).y,rpath.at(i).x,rpath.at(i).y);
		}
		std::vector<Point> kpath = getRobot()->getKalmanPath();
		dc.SetPen(wxPen(WXSTRING("BLUE"),1, wxPENSTYLE_SOLID));
		for(uint64_t i = 1 ; i < kpath.size();++i)
		{
			dc.DrawLine(kpath.at(i-1).x,kpath.at(i-1).y,kpath.at(i).x,kpath.at(i).y);
		}
		std::vector<Point> pPath = getRobot()->getParticlePath();
		dc.SetPen(wxPen(WXSTRING("RED"),1, wxPENSTYLE_SOLID));
		for(uint64_t i = 1 ; i < pPath.size();++i)
		{
			dc.DrawLine(pPath.at(i-1).x,pPath.at(i-1).y,pPath.at(i).x,pPath.at(i).y);
		}
