...
namespace Model
{
	...
	Robot::Robot() :
								...
								acting(false),
								driving(false),
								communicating(false),controller(this)
	{
	...
	std::vector<Point>& Robot::getRealPath()
	{
		return realPath;
	}
	std::vector<Point>& Robot::getKalmanPath()
	{
		return kallmanPath;
	}
	std::vector<Point>& Robot::getParticlePath()
	{
		return particlePath;
	}
	...
	void Robot::drive()
	{
		try
		{
			...
			controller.ini();
			while (position.x > 0 && position.x < 1024 && position.y > 0 && position.y < 1024 && !controller.isArrived())
			{
				controller.doNextAction();
			
				realPath.push_back(this->getPosition());
				kallmanPath.push_back(controller.getBelieveKalman());
		
				particlePath.push_back(controller.getParticleBelieve());
	...
	GoalPtr Robot::getGoal()
	{
		return goal;
	}
	void Robot::setGoal(GoalPtr goal)
	{
		goal = goal;
	}
	/**
	...
	void Robot::setFilter(FILTERS filter)
	{
